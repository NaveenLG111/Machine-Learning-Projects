...
def parse_history_input(history_text):
    return [float(x) for x in re.split(r"[,\s]+", history_text.strip()) if x]
...
def main():
    ...
    actual_value = st.number_input(
        "Optional actual y value (for error metrics)",
        value=0.0,
        step=1.0
    )

    has_actual = st.checkbox("Use actual y value for accuracy/error metrics")
    ...
    if st.button("Predict"):
        try:
            ...
            if has_actual:
                result_df["actual_y"] = actual_value
                result_df["error"] = actual_value - prediction
                result_df["abs_error"] = abs(actual_value - prediction)

            st.dataframe(result_df, use_container_width=True)

            if has_actual:
                y_true = np.array([actual_value])
                ...
                c3.metric("MAPE", "NA" if np.isnan(mape) else f"{mape:.2f}%")

                if len(y_true) > 1:
                    r2 = r2_score(y_true, y_pred)
                    st.metric("R²", f"{r2:.4f}")
                else:
                    st.info("R² is not meaningful for a single prediction. Use multiple actual/predicted rows to compute R².")
